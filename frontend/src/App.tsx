...
function App() {
  ...
  const parseGenerateJson = (text: string) => {
    let payload: { prompt?: string; caption?: string };
    try {
      payload = JSON.parse(text) as { prompt?: string; caption?: string };
    } catch (error) {
      throw new Error('JSON 解析失败，请检查格式');
    }

    if (!payload.prompt && !payload.caption) {
      throw new Error('JSON 中必须包含 prompt 或 caption 字段');
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const data = await generateImage(genJsonFile);
      setGenResult(data);
