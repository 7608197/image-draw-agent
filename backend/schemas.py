...
try:
    from .schemas_patch import PatchOperation
except ImportError:
    from schemas_patch import PatchOperation
  # 空行
  # 定义“带权重的负向词”结构
...
class ReverseResponse(BaseModel):  # 完整的 reverse 返回体
  ...
  # 空行
  # 定义“文本转结构化提示词”接口请求
class TextReverseRequest(BaseModel):
    """POST /reverse/text 的请求结构。"""
    text: str = Field(..., description="Natural language prompt to normalize")
    params: Optional[ParamsBlock] = Field(None, description="Optional generation params for structured prompt")


class AgentChatRequest(BaseModel):
    """POST /agent/chat 的请求结构。"""
    user_input: str = Field(..., description="Natural language instruction for editing the structured prompt")
    current_prompt: StructuredPrompt = Field(..., description="Current structured prompt object")


class AgentChatResponse(BaseModel):
    """POST /agent/chat 的响应结构。"""
    message: str = Field(..., description="Assistant reply summarizing the update")
    updated_prompt: StructuredPrompt = Field(..., description="Updated structured prompt object")
    prompt: Optional[str] = Field(None, description="Compiled positive prompt after the update")
    negative_prompt: Optional[str] = Field(None, description="Compiled negative prompt after the update")


class AgentConfirmOption(BaseModel):
    """Agent 确认选项。"""
    id: str = Field(..., description="Option identifier")
    label: str = Field(..., description="Option label shown to the user")


class AgentConfirmEvent(BaseModel):
    """流式确认事件载荷。"""
    confirmation_id: str = Field(..., description="Pending confirmation identifier")
    message: str = Field(..., description="Confirmation prompt shown to the user")
    summary: str = Field(..., description="Patch summary generated by the agent")
    reasoning: Optional[str] = Field(None, description="Optional reasoning text for the patch")
    operations: List[PatchOperation] = Field(default_factory=list, description="Detailed proposed operations")
    options: List[AgentConfirmOption] = Field(..., description="Available confirmation options")


class AgentConfirmRequest(BaseModel):
    """POST /agent/chat/confirm 的请求结构。"""
    confirmation_id: str = Field(..., description="Pending confirmation identifier")
    choice: str = Field(..., description="Selected option id")
    mode: Literal["accept_all", "multi", "custom_prompt", "cancel"] = Field("accept_all", description="How to handle the proposal")
    selected_operation_indexes: List[int] = Field(default_factory=list, description="Indexes of operations selected for application")
    custom_text: Optional[str] = Field(None, description="Custom follow-up instruction when the user chooses other")
...
class GenerateMeta(BaseModel):  # generate 的附加信息
    ...
    mode: Optional[str] = Field(None, description="Actual generation mode used: sd/gemini/proxy")  # 可选：最终实际生成模式
    requested_mode: Optional[str] = Field(None, description="Requested generation mode before fallback")  # 可选：原始请求模式
    fallback_used: Optional[bool] = Field(None, description="Whether generation fell back to another backend")  # 可选：是否触发回退
    fallback_from: Optional[str] = Field(None, description="Original backend that failed before fallback")  # 可选：发生回退时的原始后端
